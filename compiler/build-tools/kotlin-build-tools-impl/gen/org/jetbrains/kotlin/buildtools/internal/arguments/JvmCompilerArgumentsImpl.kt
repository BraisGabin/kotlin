...
internal class JvmCompilerArgumentsImpl(
  public val requiresForwardCompatibility: Boolean,
  ...
  override operator fun <V> `get`(key: JvmCompilerArguments.JvmCompilerArgument<V>): V {
    ...
    if(requiresForwardCompatibility) {
      return getAndMap(optionsMap[key.id], key) as V
    }
  ...
  override operator fun <V> `set`(key: JvmCompilerArguments.JvmCompilerArgument<V>, `value`: V) {
    ...
    optionsMap[key.id] = if (requiresForwardCompatibility) { setAndMap(`value`, key) } else { `value` }
  }
  ...
  override fun deepCopy(): JvmCompilerArgumentsImpl = JvmCompilerArgumentsImpl(requiresForwardCompatibility).also { newArgs -> newArgs.applyArgumentStrings(toArgumentStrings()) }
