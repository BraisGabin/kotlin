...
private fun KotlinAbiDumpTaskImpl.referenceDumps(
    ...
    val result = mutableListOf<RegularFile>()
    jvm.get().forEach { jvmTargetInfo ->
        result += if (jvmTargetInfo.subdirectoryName.isEmpty()) {
            dir.get().file(jvmDumpName)
        } else {
            dir.get().dir(jvmTargetInfo.subdirectoryName).file(jvmDumpName)
        }
    }
    ...
    result += dir.get().file(klibDumpName)
